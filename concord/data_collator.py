...
class DataCollatorForContrastivePreTraining:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        for feature in features:
            ...
            # mlm_features.append({k: feature[k][0] if k in special_keys else feature[k] for k in feature})
        ...
        if self.tree_pred_strategy == "mask":
            tree_labels = self.mask_tree_tokens(tree_labels, masked_indices, indices_replaced, indices_random)
    ...
    def mask_tree_tokens(
        self, inputs: torch.Tensor, masked_indices: torch.Tensor, indices_replaced: torch.Tensor, indices_random: torch.Tensor,
    ) -> torch.Tensor:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
...
@dataclass
class DataCollatorForContrastivePreTrainingWOTree:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        mlm_attention_mask = mlm_batch["input_ids"].ne(self.tokenizer.pad_token_id)
        mlm_input_ids, mlm_labels, masked_indices, indices_replaced, indices_random = self.mask_tokens(mlm_batch["input_ids"])
        ...
        return {"input_ids": batch["input_ids"], "attention_mask": batch["attention_mask"], "mlm_input_ids": mlm_input_ids,
                "mlm_attention_mask": mlm_attention_mask, "mlm_labels": mlm_labels}
...
@dataclass
class DataCollatorForBCB:
    tokenizer: PreTrainedTokenizer
    def __call__(self, examples: Tuple[InputDataClass]) -> Dict[str, torch.Tensor]:
        input_ids = []

        labels = []
        for e in examples:
            input_ids.append(e[0])
            labels.append(e[1])
        return {"input_ids": torch.tensor(input_ids), "labels": torch.tensor(labels)}

@dataclass
class DataCollatorForPOJ104:
    tokenizer: PreTrainedTokenizer
    def __call__(self, examples: List[InputDataClass]) -> Dict[str, torch.Tensor]:
        example_inputs = []
        labels = []
        for e in examples:
            inputs = copy.deepcopy(e[:3])
            example_inputs += [inputs]
            label = e[-1]
            labels.append(label)
        return {"input_ids": torch.tensor(example_inputs), "attention_mask": torch.tensor(example_inputs).ne(self.tokenizer.pad_token_id), "labels": torch.tensor(labels)}


@dataclass
class DataCollatorForCodeSearch:
    tokenizer: PreTrainedTokenizerBase
    padding: Union[bool, str, PaddingStrategy] = True
    max_length: Optional[int] = None
    pad_to_multiple_of: Optional[int] = None
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[
        str, torch.Tensor]:
        common_keys = ['input_ids', 'attention_mask', 'token_type_ids']
        bs = len(features)
        num_sent = len(features[0]['input_ids'])
        ...
        batch = {k: batch[k].view(bs, num_sent, -1) if k in common_keys else batch[k].view(bs, num_sent, -1)[:, 0] for k
                 in batch}


        return {"input_ids": batch["input_ids"], "attention_mask": batch["attention_mask"], "labels":[]} \
            if "labels" in feature.keys() else {"input_ids": batch["input_ids"], "attention_mask": batch["attention_mask"]}
