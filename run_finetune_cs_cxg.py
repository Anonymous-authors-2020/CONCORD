...
class Model(torch.nn.Module):
    ...
    def forward(self, code_inputs=None, nl_inputs=None):
        if code_inputs is not None:
            return self.encoder(code_inputs, attention_mask=code_inputs.ne(0)) # Note that the pad token id is 0 in our tokenizer
        else:
            return self.encoder(nl_inputs, attention_mask=nl_inputs.ne(0))
...
def convert_examples_to_features(js,tokenizer,args):
    #code
    code = js['code']
    code_tokens = code.split()[:args.code_length-2]
    ...
    nl = js['nl']
    nl_tokens = nl.split()[:args.nl_length - 2]
    nl_tokens = [tokenizer.cls_token] + nl_tokens + [tokenizer.sep_token]
    nl_ids = tokenizer.convert_tokens_to_ids(nl_tokens)
    nl_padding_length = args.nl_length - len(nl_ids)
    nl_ids += [tokenizer.pad_token_id]*nl_padding_length
...
def train(args, model, tokenizer):
    ...
    train_steps = len(train_dataloader)*args.num_train_epochs
...
def main():
    ...
    model = ConcordForEncoder.from_pretrained(args.model_name_or_path, config=config)
    model = Model(model)
